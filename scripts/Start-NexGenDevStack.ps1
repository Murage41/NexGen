...
$LogFile = Join-Path $Logs "nexgen-dev-stack.log"
...
$npmCommand = if ($WithTunnel) { "npm run dev:tunnel" } else { "npm run dev" }
$timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
Add-Content -Path $LogFile -Value "`n[$timestamp] Starting NexGen dev stack: $npmCommand"

$cmdArgs = "/c cd /d `"$Root`" && $npmCommand >> `"$LogFile`" 2>&1"
Start-Process -FilePath "cmd.exe" -ArgumentList $cmdArgs -WindowStyle Hidden

Write-Host "NexGen dev stack started in the background."
