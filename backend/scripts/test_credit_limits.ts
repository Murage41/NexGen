...
async function main() {
  ...
  try {
    ...
    assert.match((await create({ name: 'Alpha Stores' })).body.error, /phone is required/);
    assert.match((await create({ name: 'Alpha Stores', phone: '0712345678', kra_pin: '12345678901' })).body.error, /KRA PIN must be a letter, 9 digits and a letter/);
    assert.match((await create({ name: 'Alpha Stores', phone: '12' })).body.error, /valid phone number/);
    assert.equal((await create({ name: 'Alpha Stores', phone: '0712345678' }, attendantSession)).status, 403, 'only admins create customers');
    const alpha = await create({ name: 'Alpha Stores', phone: '0712 345-678', kra_pin: ' a012345678z ', credit_limit: 5000, credit_age_limit_days: 30 });
    assert.equal(alpha.status, 201, JSON.stringify(alpha.body));
    assert.equal(alpha.body.data.phone, '0712345678');
    assert.equal(alpha.body.data.kra_pin, 'A012345678Z');
    assert.equal(Number(alpha.body.data.credit_limit), 5000);
    assert.equal(alpha.body.data.credit_age_limit_days, 30);
    assert.equal(alpha.body.data.payment_terms_days, 0, 'money credits fall due the day they are given');
    const alphaId = alpha.body.data.id;
    assert.equal((await create({ name: '  alpha stores ', phone: '0722000000' })).status, 409, 'names are unique ignoring case and spaces');
    const invoiceCustomer = await create({ name: 'Gamma Transport', phone: '+254700000001', billing_mode: 'invoice', payment_terms_days: 14 });
    ...
    const gammaId = invoiceCustomer.body.data.id;

    // A customer from before phones were required stays editable without one.
    const [legacyId] = await db('credit_accounts').insert({ name: 'Beta Farm', type: 'customer', balance: 0 });
    assert.equal((await call('PUT', `/credit-accounts/${legacyId}`, desktop, { credit_limit: null, kra_pin: '' })).status, 200);
    assert.equal((await call('PUT', `/credit-accounts/${alphaId}`, desktop, { phone: '' })).body.error, 'phone is required', 'a phone on file cannot be removed');
    assert.equal((await call('PUT', `/credit-accounts/${alphaId}`, desktop, { name: 'BETA FARM' })).status, 409);
    ...
    await db('credits').insert({ customer_name: 'Beta Farm', amount: 9000, balance: 9000, shift_id: oldShift, status: 'outstanding', account_id: legacyId });
    ...
    assert.equal(legacyCredit.body.data.customer_name, 'Beta Farm');
    ...
    await db('credits').insert({ customer_name: 'Alpha Stores', amount: 3000, balance: 3000, shift_id: fortyDaysAgo, status: 'outstanding', account_id: alphaId });
    const creditCount = async () => Number((await db('credits').where({ account_id: alphaId }).count({ n: 'id' }).first() as any).n);

    const ageOnly = await credit({ account_id: alphaId, amount: 1000 }, attendantSession);
    ...
    const both = await credit({ account_id: alphaId, amount: 2500 }, attendantSession);
    ...
    assert.match((await credit({ account_id: alphaId, amount: 2500, limit_override: true }, attendantSession)).body.error, /administrator must approve/, 'an attendant cannot approve their own override');
    const overrideFor2500 = { purpose: 'credit_override', account_id: alphaId, shift_id: openShift, amount: 2500 };
    const tokenFor2500 = await pinToken(attendantSession, overrideFor2500);
    assert.match(
      (await credit({ account_id: alphaId, amount: 3000, limit_override: true, approval_token: tokenFor2500 }, attendantSession)).body.error,
      /no longer matches/,
      'approval of KES 2,500 cannot approve KES 3,000',
    );
    const approved = await credit({ account_id: alphaId, amount: 2500, limit_override: true, approval_token: tokenFor2500 }, attendantSession);
    assert.equal(approved.status, 201, JSON.stringify(approved.body));
    const override = await db('credit_limit_overrides').orderBy('id', 'desc').first();
    assert.equal(override.account_id, alphaId);
    ...
    assert.equal(override.credit_id, (await db('credits').where({ account_id: alphaId }).orderBy('id', 'desc').first()).id);
    assert.deepEqual(JSON.parse(override.breaches).map((b: any) => b.rule), ['credit_limit', 'repayment_limit']);

    const byAdmin = await credit({ account_id: alphaId, amount: 100, limit_override: true }, adminSession);
    ...
    assert.match((await credit({ account_id: alphaId, amount: 100, limit_override: true }, desktop)).body.error, /Select the approving administrator/, 'the desktop needs a PIN');
    const desktopToken = await pinToken(desktop, { purpose: 'credit_override', account_id: alphaId, shift_id: openShift, amount: 100 });
    assert.equal((await credit({ account_id: alphaId, amount: 100, limit_override: true, approval_token: desktopToken }, desktop)).status, 201);
    ...
    await call('PUT', `/credit-accounts/${gammaId}`, desktop, { credit_limit: 10000, credit_age_limit_days: 7 });
    const ninetyDaysAgo = await shiftOn(addDays(today, -90), 'closed');
    await db('invoice_consumption').insert([
      { account_id: gammaId, shift_id: ninetyDaysAgo, fuel_type: 'diesel', litres: 30, retail_price_at_time: 200, retail_amount: 6000, entry_status: 'active' },
      { account_id: gammaId, shift_id: ninetyDaysAgo, fuel_type: 'diesel', litres: 250, retail_price_at_time: 200, retail_amount: 50000, entry_status: 'reversed' },
    ]);
    const [invoiceId] = await db('customer_invoices').insert({
      account_id: gammaId, invoice_number: 'CINV-TEST-1', from_date: addDays(today, -60), to_date: addDays(today, -30),
    ...
    const within = await consume({ account_id: gammaId, fuel_type: 'diesel', litres: 5 }, attendantSession);
    ...
    const overLimit = await consume({ account_id: gammaId, fuel_type: 'diesel', litres: 15 }, attendantSession);
    ...
    const overdue = await consume({ account_id: gammaId, fuel_type: 'diesel', litres: 15 }, desktop);
    ...
    const litresToken = await pinToken(desktop, { purpose: 'consumption_override', account_id: gammaId, shift_id: openShift, fuel_type: 'diesel', litres: 15 });
    assert.match(
      (await consume({ account_id: gammaId, fuel_type: 'diesel', litres: 16, limit_override: true, approval_token: litresToken }, desktop)).body.error,
      /no longer matches/,
    );
    const approvedFuel = await consume({ account_id: gammaId, fuel_type: 'diesel', litres: 15, limit_override: true, approval_token: litresToken }, desktop);
    ...
    const alphaRow = list.find((a: any) => a.id === alphaId);
    assert.equal(alphaRow.kra_pin, 'A012345678Z');
    assert.deepEqual(alphaRow.credit_check.breaches.map((b: any) => b.rule), ['credit_limit', 'repayment_limit']);
    assert.equal(list.find((a: any) => a.id === legacyId).credit_check, undefined, 'no status for customers without limits');
    const detail = (await call('GET', `/credit-accounts/${alphaId}`, desktop)).body.data;
    ...
    assert(attendantList.find((a: any) => a.id === alphaId).credit_check, 'attendants can see a customer is over their limit');
    assert.equal((await call('GET', `/credit-accounts/${alphaId}`, attendantSession)).status, 403, 'customer history and approvals are for administrators (M6)');
